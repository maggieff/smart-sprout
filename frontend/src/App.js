...
function App() {
  ...
  const handlePlantRemove = (removedPlantId) => {
    setPlants(prevPlants => 
      prevPlants.filter(plant => plant.id !== removedPlantId)
    );
    
    if (selectedPlant && selectedPlant.id === removedPlantId) {
      ...
      <AppContainer>
        ...
        <MainContent>
          <Routes>
            ...
            <Route 
              path="/add-plant" 
              element={<AddPlant onPlantAdd={handlePlantAdd} />} 
            />
            <Route 
              path="/plant/:plantId" 
              element={
